...
class MusicFolderScan(models.TransientModel):
    ...
    def _build_image_cache(self, folder_id):
        ...
        with api.Environment.manage():
            ...
            # Lock the folder. A new cursor is necessary right after since it is closed explicitly
            # If the folder is locked, do nothing
            if not self._lock_folder(folder_id):
                return {}
            self = self._get_new_cursor()

            MusicFolder = self.env['oomusic.folder']
            Folder_child = MusicFolder.search([('id', 'child_of', folder_id)])

            i = 0
            total = len(Folder_child)
            for folder_child_id in Folder_child.ids:
            ...
            if not self.env.context.get('test_mode'):
                self.env.cr.commit()
                self.env.cr.close()
            _logger.debug("Image caching of folder_id \"%s\" completed!", folder_id)
            return {}

    def build_kanban_cache_th(self, folder_id):
        '''
        This is the method used to build the Kanban image cache with a new thread. To improve speed,
