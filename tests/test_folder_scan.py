...
class TestOomusicFolderScan(test_common.TestOomusicCommon):
    def test_00_initial_scan(self):
        ...
        self.assertEqual(
            set(Tracks.mapped("name")), set(["Song1", "Song2", "Song3", "Song4", "Song5", "Song6"])
        )
        ref_data = {
        ...
        self.assertEqual(set(Albums.mapped("name")), set(["Album1", "Album2", "Album3"]))
        ref_data = {
    ...
    def test_10_modify_tags(self):
        ...
        new_tags = {
            ...
            "Song3": {"GENRE": "GENRE4"},
            "Song4": {"GENRE": "GENRE4"},
            "Song5": {"ARTIST": "ARTIST3", "ALBUMARTIST": "ARTIST3"},
            "Song6": {"ARTIST": "ARTIST3", "ALBUMARTIST": "ARTIST3"},
        }
        Tracks = self.TrackObj.search([("root_folder_id", "=", self.Folder.id)])
        for Track in Tracks:
    ...
    def test_20_rename_dir(self):
        ...
        shutil.move(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "folder_scan_test", "Artist1"
            ),
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "folder_scan_test", "Artist3"
            ),
        )
    ...
    def test_30_remove_dir(self):
        ...
        # Verify the music.track data
        self.assertEqual(set(Tracks.mapped("name")), set(["Song5", "Song6"]))
        ref_data = {
            "Song5": (u"Artist2", u"Album3", u"Genre3", u"2003"),
            "Song6": (u"Artist2", u"Album3", u"Genre3", u"2003"),
        }
        ...
        # Verify the music.album data.
        self.assertEqual(set(Albums.mapped("name")), set(["Album3"]))
        ref_data = {"Album3": (2, u"Artist2", u"Genre3", u"2003")}
        for Album in Albums:
            self.assertEqual(
                ref_data[Album.name],
                (len(Album.track_ids), Album.artist_id.name, Album.genre_id.name, Album.year),
